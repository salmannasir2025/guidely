...
def test_ask_endpoint_with_search(mock_ask_dependencies, mock_stream_generator):
    ...
    mocks["get_llm_explanation_stream"].return_value = mock_stream_generator("This is the final AI answer.")
    mocks["log_interaction"].return_value = None

    # --- Act ---
    response = client.post(
        "/api/ask",
        json={"query": "What is FastAPI?", "language_code": "en-US", "user_id": "test_user", "mode": "tutor"}
    )
    # --- Assert ---
    ...
    mocks["get_rag_prompt"].assert_called_once_with("programming", "Mocked search result context.", "What is FastAPI?", "en-US")
...
def test_ask_endpoint_math_solver_path(mock_ask_dependencies, mock_stream_generator):
    ...
    response = client.post(
        "/api/ask",
        json={"query": "What is 21 * 2?", "language_code": "en-US", "user_id": "test_user", "mode": "assistant"}
    )
    # --- Assert ---
    assert response.status_code == 200
    # The math solver returns a single chunk
    chunk = json.loads(response.text)
    assert chunk == {"type": "result", "source": "math_solver", "chunk": "The answer is 42."}
...
def test_ask_endpoint_no_search_path(mock_ask_dependencies, mock_stream_generator):
    ...
    mocks["get_llm_explanation_stream"].return_value = mock_stream_generator("A philosophical answer.")
    mocks["log_interaction"].return_value = None

    # --- Act ---
    response = client.post("/api/ask", json={"query": "What is the meaning of life?", "language_code": "en-US", "user_id": "test_user", "mode": "tutor"})
    # --- Assert ---
    assert response.status_code == 200
    lines = response.text.strip().split('\n')
    chunks = [json.loads(line) for line in lines]
