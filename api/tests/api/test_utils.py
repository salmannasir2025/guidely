...
def test_synthesize_speech_success():
    ...
    with patch(
        ...
        response = client.post(
            "/synthesize-speech", json={"text": "Hello world", "language_code": "en-US"}
        )
...
def test_synthesize_speech_unsupported_language():
    ...
    with patch(
        "api.speech.text_to_speech",
        new_callable=AsyncMock,
        side_effect=UnsupportedLanguageError(error_message),
    ) as mock_tts:
        response = client.post(
            "/synthesize-speech", json={"text": "some text", "language_code": "ps-PK"}
        )
...
def test_translate_success():
    ...
    with patch(
        "api.llm.translate_text",
        new_callable=AsyncMock,
        return_value="یہ ایک امتحان ہے",
    ) as mock_translate:
        response = client.post(
            "/translate",
            json={
        ...
        mock_translate.assert_awaited_once_with(
            text="This is a test", source_language="English", target_language="Urdu"
        )
