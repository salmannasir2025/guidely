...
def test_get_history_success():
    ...
    mock_history = [
        {"query": "q1", "answer": "a1", "source": "llm", "timestamp": datetime.now(timezone.utc).isoformat()}
    ]
    with patch("api.database.get_user_history", new_callable=AsyncMock, return_value=mock_history) as mock_get:
...
def test_get_history_empty():
    ...
    with patch("api.database.get_user_history", new_callable=AsyncMock, return_value=[]) as mock_get:
...
def test_clear_history():
    ...
    with patch("api.database.delete_user_history", new_callable=AsyncMock) as mock_delete:
...
def test_submit_feedback():
    ...
    feedback_payload = {"user_id": "user123", "query": "q", "answer": "a", "feedback_type": "up"}
