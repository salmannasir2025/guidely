...
from api.speech import speech_to_text, text_to_speech, UnsupportedLanguageError, TranscriptionError, SynthesisError
...
@pytest.mark.asyncio
@pytest.mark.parametrize("app_code, google_code", [("ps-PK", "ps-AF"), ("pa-PK", "pa-IN")])
@patch("api.speech.speech_client")
async def test_speech_to_text_language_mapping(mock_speech_client, app_code, google_code):
    """Tests that internal language codes are correctly mapped to Google API codes."""
    mock_file = MagicMock()
    mock_file.read = AsyncMock(return_value=b"mock_audio_data")
    mock_speech_client.recognize.return_value = MagicMock(results=[]) # Don't need a full response

    await speech_to_text(mock_file, app_code)

    mock_speech_client.recognize.assert_called_once()
    assert mock_speech_client.recognize.call_args[1]['config'].language_code == google_code

@pytest.mark.asyncio
async def test_speech_to_text_unsupported_language():
    """Tests that an error is raised for an unsupported STT language."""
    with pytest.raises(UnsupportedLanguageError, match="Language 'bal-PK' is not supported for speech-to-text."):
