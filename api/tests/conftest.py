...
def mock_ask_dependencies(mocker):
    ...
    mocks = {
        "classify_query": mocker.patch(
            "api.ask.classify_query", new_callable=AsyncMock
        ),
        "perform_web_search": mocker.patch(
            "api.ask.perform_web_search", new_callable=AsyncMock
        ),
        "get_rag_prompt": mocker.patch("api.ask.get_rag_prompt"),
        "get_llm_explanation_stream": mocker.patch(
            "api.ask.get_llm_explanation_stream", new_callable=AsyncMock
        ),
        "solve_math_problem": mocker.patch("api.ask.solve_math_problem"),
        "log_interaction": mocker.patch(
            "api.ask.log_interaction", new_callable=AsyncMock
        ),
    }
    return mocks
...
def mock_health_dependencies(mocker):
    ...
    mocks = {
        ...
        "redis": mocker.patch(
            "api.cache.check_redis_connection", new_callable=AsyncMock
        ),
