...
async def synthesize_speech(request: Request, body: SynthesizeRequest):
    try:
        ...
        if not audio_content:
            raise HTTPException(status_code=500, detail="Failed to synthesize audio due to an unknown error.")
...
async def translate(request: Request, body: TranslateRequest):
    ...
    source_lang_name = LANGUAGE_MAP.get(body.source_language_code, body.source_language_code)
    target_lang_name = LANGUAGE_MAP.get(body.target_language_code, body.target_language_code)
