...
try:
    ...
    logging.error(
        f"Failed to initialize Google Speech-to-Text client: {e}. For local development, ensure you have run 'gcloud auth application-default login'."
    )
    speech_client = None
...
try:
    ...
    logging.error(
        f"Failed to initialize Google Text-to-Speech client: {e}. For local development, ensure you have run 'gcloud auth application-default login'."
    )
    tts_client = None
...
TTS_VOICE_MAP = {
    ...
    "ur-PK": texttospeech.VoiceSelectionParams(
        language_code="ur-PK", name="ur-PK-Wavenet-A"
    ),
...
async def speech_to_text(file: UploadFile, language_code: str = "en-US") -> str | None:
    ...
    if not google_language_code:
        ...
        raise UnsupportedLanguageError(
            f"Language '{language_code}' is not supported for speech-to-text."
        )
    ...
    try:
        ...
        response = await run_in_threadpool(
            speech_client.recognize, config=config, audio=audio
        )
        ...
        if response.results:
            ...
            logging.warning(
                f"Speech-to-Text returned no results for file '{file.filename}'."
            )
            return ""  # Return empty string for no transcription vs None for error

    except Exception as e:
        logging.error(
            f"An error occurred during Speech-to-Text processing for file '{file.filename}': {e}"
        )
        return None
...
async def text_to_speech(text: str, language_code: str = "en-US") -> bytes | None:
    ...
    if language_code not in TTS_VOICE_MAP:
        ...
        raise UnsupportedLanguageError(
            f"Language '{language_code}' is not supported for speech synthesis."
        )
